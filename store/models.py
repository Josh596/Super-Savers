...
class PallyManager(models.Manager):
    def get_queryset(self):
        return (
            super(PallyManager, self)
            .get_queryset()
            .filter(available_slots__gt=0, expiry_date__gt=timezone.now())
        )
...
class Price(models.Model):
    unit = models.ForeignKey(
        Unit, on_delete=models.CASCADE, related_name="related_quantity"
    )
...
class Product(models.Model):
    ...
    categories = models.ManyToManyField(
        Category, related_name="%(app_label)s_%(class)s_related", blank=True
    )
    vendor = models.ForeignKey(
        Vendor, on_delete=models.CASCADE, related_name="%(app_label)s_%(class)s_related"
    )
    ...
    price = models.OneToOneField(
        Price, on_delete=models.CASCADE, related_name="related_product"
    )
    ...
    in_season = models.CharField(
        max_length=30,
        choices=ProductSeasonalStaus.choices,
        default=ProductSeasonalStaus.IN_SEASON,
    )
...
class Pally(models.Model):
    author = models.ForeignKey(
        UserBase, on_delete=models.CASCADE, related_name="pally_creator", null=True
    )
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
    price_per_slot = models.OneToOneField(
        Price, on_delete=models.CASCADE, related_name="related_pally"
    )
