...
from store.models import Pally, Price, Product
...
def register(request):
    register_form = VendorRegistrationForm(
...
def orders_all(request):
    ...
    pally = Pally.objects.filter(product__in=product)
    # pally = vendor.store_pally_related.all()
    orders = OrderItem.objects.filter(
        product__in=product, order__billing_status=True
    ).order_by("-status")
    pally_orders = PallyOrderItem.objects.filter(
        product__in=pally, order__billing_status=True
    ).order_by("-status")
    status = {
        1: "Not Yet Shipped",
        2: "Shipped",
    ...
    return render(
        request,
        "vendor/orders-all.html",
        {"orders": orders, "status": status, "pally_orders": pally_orders},
    )
...
def edit_product(request, product_id):
    ...
    return render(
        request, "vendor/product-edit.html", {"product": product, "form": form}
    )
...
def add_product(request):
    ...
    return render(
        request,
        "vendor/product-edit.html",
        {"form": product_form, "price_form": price_form},
    )
